    ...
    var getTransformedData = function () {
      ...
      for (var x = 0; x < xlength; x++) {
        ...
        for (var y = 0; y < ylength; y++) {
          t.push(transformPoint([(x - xlength / 2) / (xlength * 1.41) * displayWidth, heights[x][y], (y - ylength / 2) / (ylength * 1.41) * displayWidth]));
        }
      }
      return output;
    };
    var renderSurface = function () {
      ...
      for (var x = 0; x < xlength - 1; x++) {
        for (var y = 0; y < ylength - 1; y++) {
          ...
          d0.push({
            path: 'M' + (data[x][y][0] + displayWidth / 2).toFixed(10) + ',' + (data[x][y][1] + displayHeight / 2).toFixed(10) +
